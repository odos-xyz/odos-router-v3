...
def test_swap_wrong_msg_value_zero(router, weth_executor):
...
def test_wrong_msg_value_nonzero(router, weth_executor):
    weth_address = weth_executor.WETH()
    input_amount = int(1e18)
...
def test_swap_with_hook(router, weth_executor):
    weth_address = weth_executor.WETH()
    input_amount = int(1e18)
...
def test_swap_positive_slippage(router, weth_executor):
    ...
    output_quote = input_amount - expected_slippage
    ...
    assert WETH.balanceOf(accounts[0]) - user_balance_before == output_quote
    assert WETH.balanceOf(router.address) - router_balance_before == expected_slippage
...
def test_swap_no_positive_slippage(router, weth_executor):
    ...
    assert WETH.balanceOf(accounts[0]) - user_balance_before == output_quote + expected_slippage
    assert WETH.balanceOf(router.address) - router_balance_before == 0
...
def test_swap_referral_fee_custom_split(router, weth_executor):
    ...
    custom_split = custom_split = random.randint(1, 10000)
...
def test_swap_permit2_hook(router, weth_executor):
    weth_address = weth_executor.WETH()
    input_amount = int(1e18)
